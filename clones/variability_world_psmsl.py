...
for region in clusters:
    ...
    c_mean, c_mean_loc = CLONETS.mean_clock(T, esc, unitFrom, unitTo)
    c_mean2, c_mean_loc2 = CLONETS.mean_clock(T, 'O', unitFrom, unitTo)
...
# fig.show()
# path = (cfg.PATHS['fig_path'] + esc + '/' + esc + '_1995_01-2006_12_' + unitTo + '_clockwise_RMS_world_mean.pdf')
# fig.savefig(path)
...
for region in clusters:
    ...
    fig2, data2, c_mean2 = CLONETS.plotTimeseriesAndMean(
        ...
        two=True)
    rms_perclock = np.array([utils.rms(data[:, i], c_mean) for i in 
    ...
    # fig, ax = plt.subplots()
    # ax.bar(np.arange(1, 145)-0.2, rms_monthly, color='tab:orange', width=0.4, label='AOHIS')
    # ax.bar(np.arange(1, 145)+0.2, rms_monthly2, color='tab:blue', width=0.4, label='Ocean only')
    # plt.title(region)
